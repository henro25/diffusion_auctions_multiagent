"""
CLIP & VLM Alignment Analysis for 2-Agent Diffusion Auctions (Multi-GPU)
...
It processes images generated by generate_images_2_agent_multigpu.py and creates individual
...
# Paths
IMAGES_DIR = "/datastor1/gdaras/diffusion_auctions_multiagent/images/images_2_agent_multigpu"
OUTPUT_DIR = (
    "/datastor1/gdaras/diffusion_auctions_multiagent/alignment/alignment_2_agent_multigpu"
)
...
class MultiGPUAnalyzer:
        ...
        for gpu_id in self.gpu_indices:
            ...
            if enable_vlm:
                try:
                    ...
                    print(f"VLM assessor loaded on GPU {gpu_id} with models: {vlm_assessor.loaded_models}")
    ...
    def get_clip_alignment_score(
        ...
        try:
            ...
            print(f"Error processing {image_path} with text '{text}' on GPU {gpu_id}: {e}")
            return 0.0
    ...
    def analyze_single_image(self, task: AnalysisTask, gpu_id: int) -> Dict:
        ...
        try:
            ...
            if task.enable_vlm and gpu_id in self.vlm_assessors and self.vlm_assessors[gpu_id]:
                try:
                    vlm_quality_result = self.vlm_assessors[gpu_id].assess_single_image(task.image_path)
                except Exception as e:
                    print(f"VLM assessment failed for {task.image_path} on GPU {gpu_id}: {e}")
                    vlm_quality_result = None
            ...
            result = {
                ...
                "welfare_metrics": {
                    ...
                    "total_welfare": agent1_score * bids[0] + agent2_score * bids[1],
                },
                "processing_info": {
                    "gpu_id": gpu_id
                }
...
def parse_image_filename(filename: str) -> Optional[Dict]:
    ...
    Expected format: idx{prompt_index:03d}_b1_{bid1:.2f}_b2_{bid2:.2f}_s{sample_idx:02d}.png
    """
    try:
        # Remove extension
        ...
        # Extract components for 2-agent format
        prompt_idx = int(parts[0].replace("idx", ""))
        bid1 = float(parts[2])
        bid2 = float(parts[4])
        sample_idx = int(parts[5].replace("s", ""))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze CLIP alignment and VLM quality for 2-agent generated images using multi-GPU"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--bid_combination",
        type=str,
        help="Specific bid combination like '1.0,0.0' (optional)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--gpu_indices", type=str, default=None,
    ...
    for image_path in image_paths:
        try:
            ...
            # Create output path
            output_filename = f"alignment_p{prompt_idx:03d}_b{bids[0]:.2f}_{bids[1]:.2f}_s{sample_idx:02d}.json"
