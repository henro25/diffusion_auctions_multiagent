"""
CLIP & VLM Alignment Analysis for 2-Agent Diffusion Auctions
...
It processes images generated by generate_images_2_agent.py and creates individual
JSON files for each prompt/bidding combination/sample.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze CLIP alignment and VLM quality for 2-agent generated images"
    )
    parser.add_argument(
    ...
    if args.enable_vlm:
        try:
            print("Loading VLM quality assessor...")
            vlm_assessor = VLMQualityAssessor(config_path=args.vlm_config)
            vlm_assessor.load_models()
            print(f"VLM assessor loaded with models: {vlm_assessor.loaded_models}")
        except Exception as e:
            print(f"Warning: Failed to load VLM assessor: {e}")
            print("Continuing with CLIP-only assessment...")
            vlm_assessor = None
    ...
    for image_path in tqdm(image_paths, desc="Processing images"):
        try:
            ...
            clip_quality_score = calculate_clip_quality(
            ...
            if vlm_assessor:
                try:
                    vlm_quality_result = vlm_assessor.assess_single_image(image_path)
                except Exception as e:
                    print(f"VLM assessment failed for {image_path}: {e}")
                    vlm_quality_result = None
    ...
    # Cleanup VLM assessor if used
    if vlm_assessor:
        vlm_assessor.cleanup()
